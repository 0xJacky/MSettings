...
#define kUrl_SunbelifeWeibo @"http://weibo.cn/x1nyang"
...
@end
@interface MSettingsPrefsRepoListController: SKTintedListController<SKListControllerProtocol>{
}
@end
...
@implementation MSettingsPrefsListController
 ...
 -(NSArray*) customSpecifiers
 {
 	return @[
 		...
 		@{ 
 			@"cell": @"PSGroupCell",
            @"footerText": @"MSettings © 2015 来自 S™ 中文源"
        },
 		];
 }

 -(void) respring {
 	UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"通过注销来使配置生效"
 		message:@"您确定现在就要注销吗？"
 		delegate:self
...
@implementation MSettingsPrefsSpringBoardSettingsListController
 ...
 -(NSArray*) customSpecifiers
 {
 	return @[
 		...
 		@{
 			@"cell": @"PSGroupCell",
 			@"footerText": @"更改设置后，您需要通过注销来使配置生效"
 		},
 		];
}
...
-(NSArray*) customSpecifiers
{
	return @[
		...
		@{
 			@"cell": @"PSLinkListCell",
 			@"label": @"S™ 中文源",
 			@"detail": @"MSettingsPrefsRepoListController",
 			@"cellClass": @"SKTintedCell",
 		},
		@{
			@"cell" : @"PSGroupCell",
...
@implementation MSettingsPrefsRepoListController
-(UIColor*) headerColor; { return [UIColor darkGrayColor]; }
-(BOOL) tintNavigationTitleText { return NO; }

- (BOOL) showHeartImage { return NO; }
-(NSString*) customTitle { return @"S™ 中文源"; }
-(NSArray*) customSpecifiers
{
	return @[
		@{
...
- (void) SunbelifeWeibo:(PSSpecifier *)specifier {
	[SKSharedHelper openWebSite:kUrl_SunbelifeWeibo];
}
