...
- (void) setupHeader {
    ...
    if ([self respondsToSelector:@selector(headerText)]) {
        ...
        {
            header.frame = CGRectMake(header.frame.origin.x, header.frame.origin.y, header.frame.size.width, header.frame.size.height + 100);

            label.frame = CGRectMake(label.frame.origin.x, 10, label.frame.size.width, label.frame.size.height + 55);
